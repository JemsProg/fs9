...
const Cart = () => {
  ...
  const updateQuantity = async (cartItem, newQty) => {
    ...
    if (newQty > cartItem.product.countInStock) {
      setError(
        `Only ${cartItem.product.countInStock} item(s) are available in stock.`,
      );
    ...
    try {
      ...
      // Reload cart so Django totals stay in sync
      await fetchCart();
    } catch (err) {
                    ...
                    <div className="flex min-w-0 items-center gap-6">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-2 text-xs font-medium">
                          Price: $
                          {Number(item.product.product_price).toFixed(2)}
                        </p>
